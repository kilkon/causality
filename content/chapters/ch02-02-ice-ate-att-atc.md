...
<table class="po-table" aria-label="ICE 관측 불가능성 예시">
      <thead>
        <tr><th>단위 \(i\)</th><th>\(T_i\)</th><th>\(Y_i(1)\)</th><th>\(Y_i(0)\)</th><th>\(\tau_i\)</th></tr>
      </thead>
      <tbody>
...
:::
    ...
    **무작위 배정 하 식별.** \(\{Y_i(1),Y_i(0)\}\perp\!\!\!\perp T_i\)이면

    \[
      \mathrm{ATE}
      = \mathbb{E}[Y_i^{\mathrm{obs}} \mid T_i=1]
      - \mathbb{E}[Y_i^{\mathrm{obs}} \mid T_i=0].
    \]
...
:::
    ...
    \[
      \mathrm{ATT}
      \equiv \mathbb{E}[\tau_i \mid T_i=1]
      = \mathbb{E}[Y_i(1)\mid T_i=1] - \mathbb{E}[Y_i(0)\mid T_i=1].
    \]
...
:::
    ...
    **관측 차이의 분해.**
    ...
    \[
      \underbrace{\mathbb{E}[Y_i^{\mathrm{obs}}\mid T_i=1]-\mathbb{E}[Y_i^{\mathrm{obs}}\mid T_i=0]}_{\text{관측 평균 차이}}
      =
      \underbrace{\mathrm{ATT}}_{\text{처치군 효과}}
      +
      \underbrace{\mathbb{E}[Y_i(0)\mid T_i=1]-\mathbb{E}[Y_i(0)\mid T_i=0]}_{\text{선택 편향}}.
    \]
...
:::
    ...
    \[
      \mathrm{ATC}
      \equiv \mathbb{E}[\tau_i \mid T_i=0]
      = \mathbb{E}[Y_i(1)\mid T_i=0] - \mathbb{E}[Y_i(0)\mid T_i=0].
    \]
...
:::
    ...
    또한 \(\{Y_i(1),Y_i(0)\}\perp\!\!\!\perp T_i\)이면 \(\mathrm{ATE}=\mathrm{ATT}=\mathrm{ATC}\)가 성립한다.
...
:::
    ...
    \[
      \mathbb{E}[Y^{\mathrm{obs}}\mid T=1]-\mathbb{E}[Y^{\mathrm{obs}}\mid T=0]
...
:::
    ...
    \[
      ...
      \mathrm{ATT}=\int \tau(\mathbf{x})\,dP(\mathbf{x}\mid T=1),\quad
      \mathrm{ATC}=\int \tau(\mathbf{x})\,dP(\mathbf{x}\mid T=0).
    \]
...
:::
    ...
    \[
      ...
      \begin{cases}
        ...
        \mathrm{ATT}=\int \tau(\mathbf{x})\,dP(\mathbf{x}\mid T=1) \\
        \mathrm{ATC}=\int \tau(\mathbf{x})\,dP(\mathbf{x}\mid T=0)
      \end{cases}
    \]
